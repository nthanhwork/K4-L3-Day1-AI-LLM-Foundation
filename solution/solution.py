...
def call_openai_mini(
    ...
    """
    #  gọi call_openai với model=OPENAI_MINI_MODEL
...
def compare_models(prompt: str) -> dict:
    ...
    """
    #  gọi call_openai và call_openai_mini, ghép dict kết quả
...
def chat_with_system_prompt(
    ...
    """
    #  giống call_openai nhưng messages có thêm phần tử role="system"
    from openai import OpenAI
    ...
    return res.choices[0].message.content or "", lat
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    """
    # dùng tiktoken để đếm token, có fallback khi lỗi
    try:
        import tiktoken
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    """
    # đếm token prompt/response, tra bảng giá, trả về dict 5 key
...
def streaming_chatbot() -> None:
    ...
    """
    #  vòng lặp while, đọc input, stream phản hồi, duy trì history
    from openai import OpenAI
...
def retry_with_backoff(
    ...
    """
    #  vòng lặp retry với exponential backoff
...
def run_assistant(
    ...
    """
    #  triển khai theo khung sườn trong docstring
    from openai import OpenAI
